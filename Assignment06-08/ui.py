        ...
        while True:
            ...
            if read_input == '1':
                ...
                if selected_statistic == '1':
                    students = [self.student_manager.retrieve_student(student_id) for student_id in
                                sorted(self.grade_manager.failing_students_ids())]
                    self.print_students_disciplines(students)
                elif selected_statistic == '2':
                    students_with_average = [(self.student_manager.retrieve_student(value[0]), value[1]) for value in
                                             self.grade_manager.best_students_ids()]
                    for value in students_with_average:
                        print('{}. {}: {}'.format(value[0].id, value[0].name, value[1]))
                elif selected_statistic == '3':
                    disciplines_with_average = [(self.discipline_manager.retrieve_discipline(value[0]), value[1])
